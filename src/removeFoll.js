...
const RemoveFoll = () => {
  ...
  const isInArtistTable = async (userId) => {
    try {
      ...
      const artistResponse = await fetch('http://10.142.0.2:8111/query', {
        method: 'POST',
        headers: {
      ...
      const listenerResponse = await fetch('http://10.142.0.2:8111/query', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (currentUserIsArtist) {
        if (isArtist) {
          if (!isFollower) {
            ...
            await fetch('http://10.142.0.2:8111/query', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              },
